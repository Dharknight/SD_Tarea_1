...
const PROTO_PATH = "./searchinfo.proto"
...
server.addService(a.InventorySearch.service, {
  GetServerResponse: (call, callback) => {
    ...
    client.connect((err, client, release) => {
      ...
      client.query(`select * from data where keywords like '%' || $1 || '%';`, [busqueda], (err, result) => {
        release()
        if (err) {
        ...
        for (i in result.rows) {
          var { id, title, description, keywords, url } = result.rows[i];
